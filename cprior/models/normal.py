...
class NormalModel(NormalInverseGammaModel):
    r"""
    Bayesian model with a normal likelihood and a normal-inverse-gamma prior
    distribution.

    Given data samples :math:`\mathbf{x} = (x_1, \ldots, x_n)`
    from a normal distribution with parameters mean :math:`\mu`, and
    ...
    .. math::
        ...
        \beta_n &= \beta + \frac{1}{2} \left(\sum_{i=1}^n (x_i - \bar{x})^2
        + \frac{n \lambda (\bar{x} - \mu_0)^2}{\lambda + n} \right).

    with prior parameters :math:`\mu_0` (loc), :math:`\lambda`
    (variance_scale), :math:`\alpha` (shape) and :math:`\beta` (scale). Note
    that :math:`n \bar{x} = \sum_{i=1}^n x_i`.
    ...
    def pppdf(self, x):
        ...
        If :math:`X` follows a normal distribution with parameters
